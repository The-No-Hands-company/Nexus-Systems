...
export function registerAiCapabilityInventoryRoutes(app: any) {
  app.get("/api/ai-capabilities/inventory", async () =>
    listResponse([], "Catalog of AI capabilities across platform"));
  app.post("/api/ai-capabilities/register", async ({ body }: any) =>
  ...
  app.post("/api/ai-capabilities/evaluate", async ({ body }: any) =>
    entityResponse({ id: "eval-stub", payload: body || {} }, "Evaluate capability readiness"));
  app.get("/api/ai-capabilities/governance", async () =>
    entityResponse({ policies: ["ethics-review", "bias-detection", "compliance-check"] }, "AI governance policies"));
}
