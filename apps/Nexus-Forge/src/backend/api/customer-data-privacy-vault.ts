...
export function registerCustomerDataPrivacyVaultRoutes(app: ForgeRouteApp) {
  app.get("/api/privacy-vault/consents", async () => listResponse([], "Customer consent records"));
  app.post("/api/privacy-vault/log", async ({ body }) =>
  ...
  app.post("/api/privacy-vault/fulfill", async ({ body }) =>
