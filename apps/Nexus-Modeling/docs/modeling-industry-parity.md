...
| Robust mesh boolean / CSG | ❌ Cosmetic *(foundation in progress)* | `BooleanOperation.cpp` still classifies whole triangles by centroid → jagged seams on coarse meshes. **Rebuild underway:** ① exact tri-tri **segment** (`TriTriIntersect`) ✓ · ② per-triangle **retriangulation** along segments (`TriangleRetriangulate`) ✓ — this also uncovered+fixed a **completely broken `ConstrainedDelaunay2D`** (returned 0 triangles for all inputs). Next: whole-mesh cut → classify → stitch → swap in |
