import { Database } from "bun:sqlite";
import { randomUUID } from "node:crypto";

export interface ConversionJob {
  id: string;
  name: string;
  jobs: string;
  inputs: string;
  outputs: string;
  properties: Record<string, unknown>;
  createdAt: string;
}

export class ConverterEngine {
  db: Database;

  constructor(p = ":memory:") {
    this.db = new Database(p);
    this.db.exec(
      "CREATE TABLE IF NOT EXISTS items (id TEXT PRIMARY KEY, name TEXT, jobs TEXT, inputs TEXT, outputs TEXT, properties TEXT DEFAULT '{}', created_at TEXT)"
    );
  }

  create(name: string, jobs?: string, inputs?: string, outputs?: string): ConversionJob {
    const item: ConversionJob = {
      id: randomUUID(),
      name,
      jobs: jobs || "",
      inputs: inputs || "",
      outputs: outputs || "",
      properties: {},
      createdAt: new Date().toISOString(),
    };
    this.db
      .prepare(
        "INSERT INTO items (id, name, jobs, inputs, outputs, properties, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)"
      )
      .run(
        item.id,
        item.name,
        item.jobs,
        item.inputs,
        item.outputs,
        JSON.stringify(item.properties),
        item.createdAt
      );
    return item;
  }

  list(): ConversionJob[] {
    return (this.db.prepare("SELECT * FROM items ORDER BY created_at DESC").all() as any[]).map(
      (r: any) => ({ ...r, properties: JSON.parse(r.properties) })
    );
  }

  get(id: string): ConversionJob | undefined {
    const r = this.db.prepare("SELECT * FROM items WHERE id = ?").get(id) as any;
    return r ? { ...r, properties: JSON.parse(r.properties) } : undefined;
  }
}
