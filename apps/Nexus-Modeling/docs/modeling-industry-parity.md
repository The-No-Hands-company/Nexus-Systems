...
| Modifier stack (ordered, non-destructive) | 3ds Max, Blender | ❌ | No modifier-stack concept at the mesh level |
...
| PLY | import + export | all | 🟡 | Export only (ASCII); import next |
...
**Verdict:** interchange was the most glaring gap and is now being closed. **OBJ import + STL import/export have landed** (with non-finite hardening and round-trip tests), so `import → edit → export` works for the two most universal mesh formats. Remaining: PLY import, then glTF 2.0 (modern standard), then FBX/USD/Alembic, then editor File▸Import/Export wiring.
...
1. **Interchange (L13)** — *in progress:* OBJ import + STL import/export landed; PLY import, glTF, and editor wiring remain. Was the top gap; now being actively closed.
...
- **IO importers + core formats:** ✅ OBJ import + STL import/export landed. Remaining: PLY import, glTF 2.0 import+export, then editor File▸Import/Export wiring. (L13)
