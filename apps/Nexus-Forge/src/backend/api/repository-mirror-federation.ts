...
export function registerRepositoryMirrorFederationRoutes(app: ForgeRouteApp) {
  ...
  app.get("/api/repository-mirror-federation/status", async () =>
    entityResponse(
      { providers: ["github", "gitlab", "gitea", "bitbucket"], syncState: "stubbed" },
      "Mirror federation status",
    ),
  );

  app.post("/api/repository-mirror-federation/stub", async ({ body }) =>
    entityResponse(
      { id: "mirror-federation-stub", ...(body || {}) },
      "Mirror federation stub request accepted",
    ),
  );
}
