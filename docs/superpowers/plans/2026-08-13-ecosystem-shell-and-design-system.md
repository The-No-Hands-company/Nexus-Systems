...
- **The app list has three states, not two.** An empty list must never be treated as "this app does not exist": before `listApps()` resolves, and permanently if it rejects, every id looks unknown and a real app renders "App not found." Distinguish loading, ready and failed, and only say not-found once the list has actually loaded.
