...
| Robust mesh boolean / CSG | ❌ Cosmetic *(foundation in progress)* | `BooleanOperation.cpp` still classifies whole triangles by centroid → jagged seams on coarse meshes. **Rebuild underway:** ① tri-tri **segment** (`TriTriIntersect`) ✓ · ② per-triangle **retriangulation** (`TriangleRetriangulate`) ✓ — also fixed a **completely broken `ConstrainedDelaunay2D`** · ③ **whole-mesh cut** (`MeshCut`, area-preserving split along A∩B) ✓. Next: classify sub-tris → keep per op → stitch → swap in; prove watertight+manifold+volume on coarse boxes |
