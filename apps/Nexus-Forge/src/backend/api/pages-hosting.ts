...
export function registerPagesHostingRoutes(app: ForgeRouteApp) {
  ...
  app.get("/api/pages-hosting/deployments", async () =>
    entityResponse(
      { source: "branch-or-ci", deploymentHistory: "stubbed" },
      "Pages deployment status",
    ),
  );

  app.post("/api/pages-hosting/stub", async ({ body }) =>
    entityResponse(
      { id: "pages-hosting-stub", payload: body || {} },
      "Pages hosting stub request accepted",
    ),
  );
}
