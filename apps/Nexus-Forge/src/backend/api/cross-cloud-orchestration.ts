...
export function registerCrossCloudOrchestrationRoutes(app: any) {
  app.get("/api/cross-cloud/inventory", async () =>
    listResponse([], "Cross-cloud resource inventory"));
  app.post("/api/cross-cloud/deploy", async ({ body }: any) =>
  ...
  app.post("/api/cross-cloud/failover", async ({ body }: any) =>
