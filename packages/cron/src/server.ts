...
export function createServer() {
  ...
  const baseUrl = process.env.NEXUS_NEXUS_CRON_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new CronEngine("data/nexus-cron.sqlite");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled: (process.env["NEXUS_CRON_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
      ...
      if (req.method === "POST" && p === "/api/v1/items") {
        ...
        return json(engine.create(b.name, b.jobs, b.schedules, b.executions), 201);
      }
  ...
  console.log(`[nexus-cron] Listening on port ${server.port}`);
