...
import { ClipboardEngine } from "./clipboard-engine";
...
export function createServer() {
  ...
  const baseUrl = process.env.NEXUS_NEXUS_CLIPBOARD_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new ClipboardEngine("data/nexus-clipboard.sqlite");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled: (process.env["NEXUS_CLIPBOARD_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
      ...
      if (req.method === "POST" && p === "/api/v1/items") {
        ...
        return json(engine.create(b.name, b.items, b.groups, b.tags), 201);
      }
  ...
  console.log(`[nexus-clipboard] Listening on port ${server.port}`);
