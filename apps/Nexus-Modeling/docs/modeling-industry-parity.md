...
| Robust mesh boolean / CSG | ❌ Cosmetic *(foundation in progress)* | `BooleanOperation.cpp` still classifies whole triangles by centroid → jagged seams on coarse meshes. **Rebuild underway:** exact tri-tri intersection **segment** primitive landed (`TriTriIntersect`, orient3D-exact); next: CDT retriangulation along segments → classify → stitch → swap in |
