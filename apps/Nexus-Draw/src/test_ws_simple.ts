...
const server = Bun.serve({
  ...
  websocket: {
    open(ws) { console.log("WS OPEN", ws.data); collab.open(ws, (ws.data as any).boardId); },
    message(ws, data) { console.log("WS MESSAGE", data.byteLength); collab.message(ws, data); },
    close(ws) { console.log("WS CLOSE"); collab.close(ws); },
  },
});
