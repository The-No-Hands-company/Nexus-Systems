    ...
    void processAudio(AudioBuffer& outputBuffer, const AudioBuffer* inputBuffer = nullptr) {
        if (!m_running || !m_voiceManager) {
            ...
            for (uint32_t ch = 0; ch < outputBuffer.getChannelCount(); ++ch) {
                float* channelData = outputBuffer.getChannelData(ch);
                for (uint32_t i = 0; i < outputBuffer.getSampleCount(); ++i) {
                    channelData[i] = 0.0f;
                }
            }
            return;
        ...
        // Apply master gain (placeholder)
        float masterGain = m_volume;
        for (uint32_t ch = 0; ch < outputBuffer.getChannelCount(); ++ch) {
            float* channelData = outputBuffer.getChannelData(ch);
            for (uint32_t i = 0; i < outputBuffer.getSampleCount(); ++i) {
                channelData[i] *= masterGain;
            }
        }
