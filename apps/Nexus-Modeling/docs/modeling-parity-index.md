...
| **Non-finite input hardening** | Solid | Foundation sweep verified: every public geometry op rejects non-finite scalars via IEEE-754 bit inspection (bit inspection, not std::isfinite — which -ffast-math makes report NaN/Inf as finite; that flag is now out of the build, and the bit check holds either way). Consolidated into a canonical geometry::isFinite in Tolerance.h with a fast-math regression-guard test. |
