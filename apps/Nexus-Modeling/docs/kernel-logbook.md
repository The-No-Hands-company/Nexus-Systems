...
- **Part V — Going back to the beginning**
  ...
  - 67. A boundary predicted in advance
...
## 54. The second operation

The corpus had gone quiet. Watertight-or-empty: no failures. Volume conservation: no violations. Per-face area coverage: no mismatches. Three chapters of measurement and the fuzzer had nothing left to say.

That is a statement about the generator at least as much as about the kernel. Two thousand configurations, and every one of them performs exactly **one** Boolean on two primitives — which is not what anyone does with a modelling kernel, and which has a specific blind spot: two primitives never share a surface.

Chaining is the ordinary case. Cut a slot, then round a corner, then subtract a bore. And the second operation is a different problem from the first, because the result of the first *carries its operands' own faces* — so the second is full of exactly coincident geometry.

Better still, chains come with oracles whose answers are known outright, which single operations do not. B is contained in A ∪ B. So

> (A ∪ B) ∩ B must be B, and (A ∪ B) ∪ B must be A ∪ B.

No tolerance to argue about, no reference implementation to trust. Over four hundred random pairs, the first of those came back **empty in eighty-eight of a hundred and fifty-one chains**. Empty is a permitted answer to the watertight-or-empty contract, which is exactly why nothing had noticed: the invariant that had been guarding this kernel for chapters is satisfied by returning nothing at all. It is a safety property, not a correctness one.

And the split was clean: every all-planar chain was right, every chain with a curved operand was wrong.

**Two defects, both the same mistake in different clothes: reading a curved surface as though it were a plane.**

The first is a single line. `Surface::normal` is a normal only for a plane; on a cylinder and a cone it is the **axis**. The Boolean decides whether a coincident face pair has its two interiors on the same side by probing a point just inside the face, along −n. On a cylinder that probe slid *along the axis* rather than into the material. Not a slightly wrong direction — a perpendicular one. The tessellator carries a comment about this exact confusion, added after it broke the centred cylinder-through-box Boolean. The Boolean had it too, in two places, and nobody had gone looking.

The second is more interesting, because fixing the first did not help. Even aimed correctly, the probe asks `classifyPoint`, and `classifyPoint` answers from the **tessellation**. A point on a curved surface is never on the chordal hull that approximates it. Measured on a sixteen-segment cylinder of radius 0.7: near the rims the hull reaches 0.6997, but midway along a facet it dips to 0.6866. A probe placed a thousandth inside the true surface is thirteen thousandths *outside* the hull. The probe was not noisy or marginal. It was reliably, systematically wrong, and no tolerance would have rescued it.

> The recurring shape of this book is a decision read from the wrong copy of something. The fan apex read float positions where doubles were available. The convexity turns read the same. Here a question about exact analytic surfaces was being put to a triangle soup that exists only to approximate them. The tessellation is downstream of the truth, and answers taken from it are answers about the approximation.

So both questions are now answered from the surfaces. A new `faceContainingPoint` locates the face a point lies on analytically — on the surface, inside the boundary, holes excluded — and the same-side question, which is about *orientation*, is settled by the dot product of the two faces' outward normals at the shared point. Neither depends on how anything happens to be tessellated.

One wrinkle worth recording, because it cost a wrong turn. Containment on a curved patch was tested in the surface's own (u,v), and every one of these parametrisations is singular somewhere — a sphere's at ±uAxis, which `makeSphere` puts on X while the grid's poles are on Z, so the singularity sits in the middle of ordinary faces. Ninety-two of a sphere's ninety-six faces placed correctly and four did not. Testing in the tangent plane instead has no singularity and fixed the sphere — and broke two cylinder chains, because a tangent projection folds on a patch spanning more than a hemisphere, which a cylinder's side face can do. Accepting *either* test kept the cylinder failures, so the false answers were the tangent plane's, not the (u,v)'s. Each frame is used where it is sound: the tangent plane on spheres, (u,v) elsewhere.

**What was proven.** Over the same four hundred pairs, (A ∪ B) ∩ B went from sixty-three correct to a hundred and seventeen, with the empties falling from eighty-eight to thirty-four; (A ∪ B) ∪ B went from a hundred and twenty-three correct to a hundred and fifty-one, with the empties falling to **none**. The single-operation corpus is untouched: the same 2107 sews, no invariant failures, no conservation violations.

**One measurement that looked like a regression and was not.** Nine of the newly-succeeding chains reported a wrong volume — until both volumes were taken two ways. The *tessellated* volumes agree to the last digit in all nine; it is the *analytic* mass properties that disagree, by a constant 1.754 × 10⁻² whenever a cylinder is involved. The chains are right and the oracle is wrong, which is now its own recorded lead rather than a blemish on this one.

Two thousand five hundred and ninety-four tests ran; two thousand five hundred and eighty-nine passed, with five hardware skips.

---

## 55. Inside out

The lead from the previous chapter was a number that could not be true: the union of a two-by-two-by-two box with a sphere, reporting a volume of 1.08. It contains the box. Nothing below eight is possible.

Its tessellation said 9.85, correctly. So the *analytic* mass properties were wrong — the path that exists precisely so that a sphere reports its exact volume rather than a faceted one, by integrating each curved face over its parameter domain instead of over its triangles.

Instrumenting the integration made the shape of it plain immediately: all one hundred and twelve of the union's spherical patches contributed a **negative** volume term. Not some of them, not the ones near a seam. Every one. The sphere's whole contribution was being subtracted from the box instead of added to it, and 8 − 7.24 ≈ 1.08 is exactly the number on the tin.

The patch normal is **du × dv**, and it was handed to the integrator flipped only by the face's `reversed` flag — which is to say, on the assumption that du × dv points *out of the solid* whenever a face is not reversed. That assumption holds for a primitive as built. It is not a property the kernel maintains anywhere: a Boolean's `fromFaces` rebuilds the result's surface records, and nothing in that path preserves which way the parametrisation runs.

Which is why no primitive test could ever have caught it. Every face of a sphere, cylinder or cone as constructed agrees with the assumption, so the sign was constant and invisible — at any radius, any segment count, translated anywhere. It takes a Boolean's fragments to put a patch on a body whose parametrisation runs the other way.

The first repair I built was wrong, and the way it was wrong is worth keeping. I reasoned that since du × dv is fixed by the parametrisation, the patch's actual facing must be carried by the **winding** of its (u,v) polygon — and the code was taking the absolute value of each parameter triangle's area, throwing that winding away. Making the area signed halved the failures across the corpus and cut the worst error seventeenfold.

It also broke three tests that had been exact for chapters: a box bored by a cylinder, where D + I = A held to 10⁻⁵ and now missed by half a unit. On those faces the assumption *did* hold, and the (u,v) winding was merely an artefact of which way the ring happened to be traversed. Two plausible stories, each right about half the corpus.

> Both stories were guesses about where the orientation lives. The face already knows: its ring is wound counter-clockwise seen from outside, so its Newell normal points out of the solid. That is topology, not a convention anyone has to maintain, and it is the same answer whichever way the surface record was rebuilt.

So the direction is read from the ring, compared once against du × dv at the parameter centroid, and flipped if they disagree. It subsumes `reversed` entirely — a reversed face has a reversed ring — and the parameter weight goes back to being an area, which is what it is.

One consequence needed catching separately. Surface *area* has no orientation, so where volume wants the signed thing, area wants the magnitude; the two now say so explicitly rather than sharing an unstated convention.

**What was proven.** Across eight hundred configurations — 2443 bodies — the count whose analytic volume disagrees with its own fine tessellation falls from **251 to 94**, and the worst disagreement from 7.6 × 10² to 1.3 × 10¹. The analytic conservation identity U + I = A + B, which had never been measured because the analytic path had never been trusted, goes from **286 violations of 624 to 84**, and its worst from 0.98 to 0.028. Primitives are untouched and still exact at every segment count.

**And what was not.** Ninety-four bodies still disagree, and the sweep in the new test bounds that on both sides so it can neither grow nor be quietly fixed without the bound being retired. There is more than one thing wrong in that integrator; this was the largest.

A note on the previous chapter, which flagged nine chained Booleans whose volumes looked wrong and concluded the chains were right and the oracle was broken. That was the correct call, and this is the broken oracle.

Two thousand five hundred and ninety-eight tests ran; two thousand five hundred and ninety-three passed, with five hardware skips.
...
The previous chapter left the remaining decline described as "the corner regions between the square and the circle assemble wrongly". That is a symptom wearing the clothes of a cause, and this chapter is about replacing it.

Two attempts came first, and the more useful of them failed. The imprint required a cut line to cross a face's boundary **exactly twice** — one entry, one exit — which is true of a convex face and false of everything else, since a line crosses once for every time it enters or leaves material and a face that has already been cut is routinely re-entered. Measured on the failing fixture: of 1248 cut attempts, 841 had two crossings and a hundred and thirteen had four, five or six, and every one of those declined. One refusal anywhere leaves the imprint incomplete and the whole Boolean empty, so this looked like the answer.

It was not. Ordering the crossings along the curve and cutting the first pair that actually bounds material — letting the driver re-offer the surface, so *k* segments are consumed in *k* passes — moved the success rate from 75.1% to **75.4%**. Three configurations out of nine hundred and seventy-two.

> The change is right and the expectation was wrong, and those are separate facts. A false statement about geometry sitting in the middle of the imprint is worth removing whether or not it is the blocker — chiefly so it stops sending the next investigation the way it sent this one.

The odd crossing counts, incidentally, are not noise: a crossing landing exactly on a vertex is reported once where a transversal one is reported twice. That is why the pair that bounds material has to be *tested* rather than inferred from parity.

So back to measurement, and this time at the sew rather than the cut. The offering came out with three directed edges used twice and nine unpaired, all of them in the corner regions where the square's boundary and the bore's circle cross at eight points per cap. Which is where the previous description came from — and it is still only a description.

The cause turned out to be visible from the geometry alone, if you look at the right cell. The four cut lines divide each cap into nine cells, and the one that matters is the **centre** one, `[−h, h]²`, for a bar of half-width h. What decides everything is what its *material* looks like once the bore's disk is removed from it. Its corner sits at h·√2, so there are exactly four regimes:

| | centre cell's material | |
|---|---|---|
| h·√2 ≤ 0.5 | the cell is inside the disk: none at all | sews |
| 0.5/√2 < h < 0.5 | **four disconnected slivers** | declines |
| h = 0.5 | four slivers pinched at the tangencies | declines |
| h > 0.5 | a square with a hole, connected | sews |

That table was written from the geometry before it was run. It then predicted the outcome for all fourteen widths tested — including both sharp transitions, where 0.3535 sews and 0.3540 does not, and 0.50 does not and 0.51 does. Fourteen of fourteen.

A boundary that sharp, predicted in advance rather than fitted afterwards, is what separates a cause from a description. And the cause is not about corners at all:

> **The imprint's face-splitting model cannot produce a face whose material is disconnected**, and this band requires exactly that.

Every cut it knows how to make is a chord between two boundary vertices, and a chord splits a connected region into two connected regions. Nothing in that vocabulary can say "these four slivers are one cell's worth of material". Closing it means partitioning a face by all of its cut curves at once — a planar arrangement — or detecting a disconnected result and emitting one face per component. That is an architectural change, and naming it accurately is worth more than a fourth attempt aimed at the wrong layer.

**What was proven.** The rule is now a test, asserting both sides: the declining widths return cleanly empty and stay closed if they ever return anything, while the sewing widths are held to real volume oracles — unchanged where the bar lies inside the bore and removes nothing, box-minus-bar where it swallows the bore. The test fails when the arrangement work lands, and its message says to promote the declining rows when it does.

Two thousand six hundred and forty-eight tests ran; all passed, with five hardware skips.

## 68. The pair that was never in the table

The previous chapter ended by naming an architectural gap and declining to attack it with a fourth wrong-layer patch. So this one starts somewhere else: at the fuzzer, and at what it *cannot generate*.

The chained-Boolean sweep — results fed back in as operands, which is the one thing the corpus had never done — reported something worth looking at. Across 3,542 chained steps it found no leaky body, no integrity failure and no non-determinism, which is the contract holding. But 1,699 of those steps, 48%, returned empty. Broken down by operation, one row did not belong:

| | steps | empty |
|---|---|---|
| drill ∪ body | 556 | 202 (36.3%) |

A union cannot be empty. The drill is aimed at the body's bounding-box centre and is three times its span, so the two solids provably overlap, and the union of two overlapping solids has volume. Something was answering "nothing" to a question with an obvious answer.

Isolating it split cleanly: with a holed operand, 89.3% empty — that is the arrangement gap from the previous chapter, already named. With a *plain* operand, 23.9%. That second number had no owner.

The first guess was wrong, which is worth recording because the guess was reasonable. A rod piercing a face makes a closed intersection loop, i.e. a hole, so perhaps hole *creation* was the blocker. Written down, then tested: a box against a rod, contained, piercing, straddling an edge. All five configurations sewed, the piercing ones producing forty faces with two holed faces among them. Hole creation was fine.

Narrowing by primitive kind instead put the whole thing in one place.

| | union | difference |
|---|---|---|
| box + coaxial rod | solid | solid |
| cylinder + coaxial rod | solid | solid |
| sphere + on-axis rod | solid | solid |
| **cone + coaxial rod** | **EMPTY** | **EMPTY** |
| sphere + off-axis rod | EMPTY | — |

The cone failed at every offset tried. The sphere failed only when the rod was off its axis — and that is the tell, because a cylinder whose axis passes through a sphere's centre cuts it in two *circles*, while one that misses cuts a quartic. The two rows are the same phenomenon seen from either side of one boundary: **axially symmetric configurations have a closed-form section, and the code either has that closed form or it does not.**

`intersectSurfaces` is a dispatch table, and reading it settled the matter in one glance. Plane against plane, sphere, cylinder and cone; sphere against sphere; cylinder against cylinder; sphere against cylinder. Then a fall-through to `Unsupported`. The cone was the one quadric that never got its own row against another curved surface.

What makes this a foundation defect rather than a missing feature is the next hop:

```cpp
case SurfaceIntersectionKind::None:
case SurfaceIntersectionKind::Point:
case SurfaceIntersectionKind::Unsupported:
    break;  // nothing to imprint
```

`Unsupported` and `None` share an arm. "I cannot express this intersection" and "these surfaces do not meet" produce identical behaviour, and the enclosing function then returns **true**. So the imprint did no work on two interpenetrating solids and reported success; the sew found faces still straddling a seam nobody had cut; and the watertight-or-empty invariant did its job and returned a clean empty body. Every layer behaved exactly as designed, and the result was a wrong answer with no diagnostic anywhere. Instrumenting it showed the shape plainly: `imprint=ok  A:96→96  B:18→18`.

> A silent no-op that reports success is worse than a failure. The contract said "watertight or empty" and it was honoured — the empty was just not the truth.

**The three pairs.** A cone stores its apex as the origin, its axis pointing apex→base, and its **slope** in the radius field, so the ring radius at axial distance *t* is `slope·t` and only `t > 0` is on the nappe. That last clause does most of the work.

- **cone ∩ cylinder**, coaxial with the axis through the apex: the nappe reaches the bore's radius at exactly one station, `t = radius/slope`. One circle.
- **cone ∩ sphere**, centre on the axis: substituting the nappe's radius gives `(1+slope²)t² − 2dt + (d² − R²) = 0`. Two roots, and the filtering is the interesting part — a sphere large enough to swallow the apex puts one root behind it, where there is no surface. Returning that root would imprint a phantom ring on the mirror cone the representation does not include, so roots are filtered rather than assumed to arrive in pairs, and such a sphere correctly yields **one** ring.
- **cone ∩ cone**, apexes on a shared axis: the station where the two radii agree. Collinear axes may point opposite ways, so an apex-to-apex pair is handled rather than mirrored into nonsense; equal slopes in the same direction are parallel nappes that never meet; and a shared apex with equal slope is not an intersection at all but the *same surface*, reported the way cylinder∩cylinder already reports its coincident case.

Everything else still says `Unsupported`, and that remains the honest answer — a quartic space curve is neither a Line nor a Circle, and claiming one would put geometry into a body that does not lie on its own surface. The change is not that the cone became fully supported. It is that it stopped being declined on **both** sides of the line every other curved pair is already split along.

One more thing had to be fixed to make any of this checkable. `surfaceDistance` — the helper whose entire stated purpose is verifying that an intersection curve lies on both input surfaces — returned `1e30` for a cone. The one surface that most needed measuring was the one it could not measure, and a verification that cannot fail is not a verification.

**What was proven.** The volumes are checked against integrals worked out by hand, sharing no code with the kernel that computed them. Cone minus a coaxial rod is `π∫₋₁^0.4((1−z)²/4 − 0.09)dz = π·0.5226667`; measured, within 5e-08 relative. Cone plus the same rod, within 4e-08. Two coaxial cones of *different* slope — and the slopes must differ, of which more in a moment — union across their crossing ring to `π·1.59375`, confirmed twice by integrating the maximum radius directly and by `A + B − A∩B`. Cone minus an on-axis sphere removes 0.142008 against a predicted 0.142001. On the guaranteed-overlapping union sweep, plain operands went from **23.9% empty to 0.0%** — seventy-four failures to none.

Two existing tests had pinned the old behaviour as permanent, and both were wrong in the same way: each used a sphere centred on the cone's axis, which genuinely does cut the nappe. They now assert the real boundary instead of the old limit.

And the closing note is the one that keeps recurring in this Part, arriving this time from the opposite direction. The fuzzer that *found* this defect could not see it fixed. Adding the three pairs moved its corpus by **exactly zero steps** — same 3,542, same 1,699 empties, byte for byte — because every drill it generates is jittered off-axis, and off-axis is precisely the quartic case that is still declined. It could reach the bug only because a cone's *base plane* is planar; it could never reach the fix.

> A generator that always perturbs cannot produce the aligned case, and the aligned case is where the closed forms live. Coaxial holes are not an exotic input — they are what a machinist drills.
