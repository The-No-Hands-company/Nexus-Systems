...
export function registerCiCdFullDepthRoutes(app: any) {
  ...
  app.get("/api/ci-cd-full-depth/deploy-tracking", async () =>
    entityResponse({ environments: ["dev", "staging", "prod"], approvals: "stubbed" }, "Deployment tracking baseline"));

  app.post("/api/ci-cd-full-depth/stub", async ({ body }: any) =>
    entityResponse({ id: "ci-cd-depth-stub", ...(body || {}) }, "CI/CD depth parity stub request accepted"));
}