import { Database } from "bun:sqlite";
import { randomUUID } from "node:crypto";

export interface DashboardWidget {
  id: string;
  name: string;
  widgets: string;
  panels: string;
  data: string;
  properties: Record<string, unknown>;
  createdAt: string;
}

export class DashboardEngine {
  db: Database;

  constructor(p = ":memory:") {
    this.db = new Database(p);
    this.db.exec(
      "CREATE TABLE IF NOT EXISTS items (id TEXT PRIMARY KEY, name TEXT, widgets TEXT, panels TEXT, data TEXT, properties TEXT DEFAULT '{}', created_at TEXT)"
    );
  }

  create(name: string, widgets?: string, panels?: string, data?: string): DashboardWidget {
    const item: DashboardWidget = {
      id: randomUUID(),
      name,
      widgets: widgets || "",
      panels: panels || "",
      data: data || "",
      properties: {},
      createdAt: new Date().toISOString(),
    };
    this.db
      .prepare(
        "INSERT INTO items (id, name, widgets, panels, data, properties, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)"
      )
      .run(
        item.id,
        item.name,
        item.widgets,
        item.panels,
        item.data,
        JSON.stringify(item.properties),
        item.createdAt
      );
    return item;
  }

  list(): DashboardWidget[] {
    return (this.db.prepare("SELECT * FROM items ORDER BY created_at DESC").all() as any[]).map(
      (r: any) => ({ ...r, properties: JSON.parse(r.properties) })
    );
  }

  get(id: string): DashboardWidget | undefined {
    const r = this.db.prepare("SELECT * FROM items WHERE id = ?").get(id) as any;
    return r ? { ...r, properties: JSON.parse(r.properties) } : undefined;
  }
}
