...
export function createServer() {
  ...
  const baseUrl = process.env.NEXUS_NEXUS_CHRONICLE_BASE_URL || `http://localhost:${port}`;
  const startedAt = Date.now();
  const engine = new ChronicleEngine("data/nexus-chronicle.sqlite");
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        return json(
          {
            ...
            cloudIntegration: {
              enabled: (process.env["NEXUS_CHRONICLE_ENABLE_CLOUD_INTEGRATION"] || "true") !== "false",
      ...
      if (req.method === "POST" && p === "/api/v1/items") {
        ...
        return json(engine.create(b.name, b.events, b.sessions, b.traces), 201);
      }
  ...
  console.log(`[nexus-chronicle] Listening on port ${server.port}`);
