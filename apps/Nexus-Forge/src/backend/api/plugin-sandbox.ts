...
export function registerPluginSandboxRoutes(app: any) {
  app.get("/api/plugins/registry", async () => listResponse([], "Plugin registry is stubbed."));
  app.post("/api/plugins/registry", async ({ body }: any) =>
    entityResponse({ id: "plugin-stub", ...(body || {}) }, "Plugin registration is stubbed.")
  );

  app.get("/api/plugins/sandboxes", async () => listResponse([], "Plugin sandbox pool is stubbed."));
  app.post("/api/plugins/sandboxes", async ({ body }: any) =>
