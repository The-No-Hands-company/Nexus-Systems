...
export async function registerWithCloud(config: Config): Promise<void> {
  try {
    ...
    const toolPayload = {
      ...
      capabilities: ["auth", "routing", "ai-provider-selection", "telemetry", "federation-aware"],
      phantom_did: PHANTOM_DID,
      version: "0.1.0",
    };

    await axios.post(`${cloudUrl}/api/v1/tools`, toolPayload, {
      headers: {
    ...
    logger.info({ tool_id: ROUTER_ID, cloud_url: cloudUrl }, "Registered with Nexus-Cloud");
    ...
    setInterval(
      async () => {
        try {
          await axios.post(
            `${cloudUrl}/api/v1/tools/${ROUTER_ID}/heartbeat`,
            {},
            {
              headers: {
                Authorization: `Bearer ${apiKey}`,
